...
//  }
  ...
  def valSplit(x:Array[DenseVector[Double]] =_X)= {
    ...
    for (i <- 0 until nlevel) { //i 是 第几类的类别循环，有几类循环几次
      for (groups <- _dataGroups.values) { //groups里面是每一组的情况，已经区分好了的
        ...
        groups.foreach(t=>{
          if (t._2 == i) {
            weightbuf += spweight_dvec(t._3)//方向错了，按行的，所以要在里面挑
          }
        })
        spweight_groups += weightbuf.toArray
        weightbuf.clear()
      }
      println(arrbuf.toVector)
  ...
  def selectVal(x:Array[DenseVector[Double]] =_X, idx:Int,row:Int,col:Int)={
    val mat= DenseMatrix.zeros[Double](row,col)
