...
object SpatialAutoCorrelation {
   ...
   * @return 全局莫兰指数，峰度
   */
  def globalMoranI(featRDD: RDD[(String, (Geometry, mutable.Map[String, Any]))], property: String, plot: Boolean = false, test: Boolean = false, weightstyle: String = "W"): String = {
...
//    (moran_i, kurtosis)
  ...
  RDD[(String, (Geometry, Map[String, Any]))] = {
    ...
    val featRDDidx = featRDD.zipWithIndex()
    featRDDidx.map(t => {
